...
```{r PCA reg p2}
#pcadata_pcr_rot = PCRxform(pcadata_w_ipc,"IncomePerCap",TRUE) 
pcadata_rot_reg_pc1 <- lm(IncomePerCap ~ PC1, data = pcadata_pcr_rot)
summary(pcadata_rot_reg_pc1)

pcadata_rot_reg_pc2 <- lm(IncomePerCap ~ PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10, data = pcadata_pcr_rot)
summary(pcadata_rot_reg_pc2)
```
