...
reg.best10 <- regsubsets(IncomePerCap~. , data = full_2017_varOfInterest, nvmax = 17, nbest = 2, method = "exhaustive")  # leaps::regsubsets() - Model selection by exhaustive (default) search, forward or backward stepwise, or sequential replacement
...
```{r}
reg.back17 <- regsubsets(IncomePerCap~. , data = full_2017_varOfInterest, nvmax = 17, nbest = 2, method = "backward")
...
summaryRegForward = summary(reg.seqrep)
# Adjusted R2
subsets(reg.seqrep, statistic="adjr2", legend = FALSE, min.size = 2, main = "Adjusted R^2")
```
