...
reg.best10 <- regsubsets(IncomePerCap~. , data = census_2017 %>% select(Men:Unemployment), nvmax = 17, nbest = 2, method = "exhaustive")  # leaps::regsubsets() - Model selection by exhaustive (default) search, forward or backward stepwise, or sequential replacement
...
```{r}
loadPkg('olsrr')
model <- lm(IncomePerCap ~ ., data = census_2017 %>% select(Men:Unemployment))
