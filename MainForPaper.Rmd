...
# Content
First, an examination was conducted on how the US Census Bureau database is structured, and which variables were included. Secondly, an analysis of Cato Institute's Freedom in the 50 States shows whether or not it worked as a potential meaningful independent variable. Later on, the groups of independent variables and how each of them could affect the income per capita of a community is presented. Then, an exploratory data analysis and some statistical tests are made to evaluate the significance of our variables and a first preview of a regression analysis. Finally, a conclusion looks into further challenges and questions that will impact upcoming analyses.
...
## What analysis has already been completed related to the content in this dataset?
An analysis using this dataset was uploaded to Kaggle in 2018. It is titled "Child Poverty in New York City: A Comprehensive Study" and analyzed child poverty rates in New York City and how they vary by location and other factors (BigironSphere, 2018). Although it was conducted using the same dataset, it focused only in New York city and not the entire country.

Another Kaggle user conducted a study titled "Comprehensive Look at the 2015 ACS estimate" (Pstetz, 2018), where he made a descriptive analysis for all the ACS variables. However, this study did not emphasize a particular variable over another one. 

Several articles have been written using the Freedom in the 50 States dataset. Most of them coming from the Cato Institute or other institutions alike such as Mercatus Center from George Mason University. 
...
An analysis of correlation between and within the predictors was performed and resulted in the correlation matrix above.  From this matrix, it was apparent that there was large positive correlation between certain variables within the “freedom” category.  This was especially apparent between economic and overall freedom scores, where the correlation coefficient was 0.98, but was also notable between Fiscal/Economic, RegulatoryPolicy/Economic, RegulatoryPolicy/Overall and FiscalPolicy/Overall with correlation coefficients ranging from 0.87 to 0.9. The correlation between Fiscal/Regulatory Policy was notable as well, with an r of 0.62.  The respective correlation coefficients between all of these variables was positive, indicating that an increase in one type of freedom corresponded to an increase in the other.  This is likely because overall freedom was made up of the sum of regulatory, fiscal and personal scores, and economic freedom was the sum of regulatory and fiscal scores.  There was some correlation between the different ethnicities and freedom variables as well, but at much lower magnitude.
...
Within the work category, there was some negative correlation between professional and multiple variables including service, construction, production and unemployment.  This implied that census tracts with larger proportions of professional workers had smaller proportions of individuals employed in service, construction or production fields or not employed at all.  This was not surprising since theoretically these proportions should sum to 1. It was thus stranger to note that there was a positive correlation between Service work and Unemployment, with a value of 0.47.
...
Within the ethnicity category, there was some negative correlation between White and the two variables Hispanic and Black.  This implied that census tracts with larger proportions of White individuals in turn had smaller proportions of Hispanic and Black individuals.  It is worth saying here that there was a negative correlation between all of the ethnic variables because the values sum to one and as one percentage increased the sum of the others must decrease.


# Scatter PLots

Scatter plots of a few of the more significant variables according to the correlation matrix were created in order to form a better understanding of the relationships.  The plots displayed three variables: Income per capita on the vertical axis, the work variable on the horizontal axis, and the color of each data point indicating the percentage of White individuals in that particular census tract.
...
The first scatter plot laid out IncomePerCap and the percent White as it related to professional work.  The trend of the data points from bottom left to upper right indicated that a census tract with more Professional individuals tended to have a larger Income Per Capita.  The color of the data points as they moved from the bottom left to the upper right became lighter, which implied that census tracts with medium or high percentages of White individuals had more medium or high income per capita.
...
The second scatter plot laid out IncomePerCap and percent White as it related to service work.  The trend of the data points from top left to bottom middle indicated that a census tract with fewer Service workers tended to have a larger income per capita.  The color of the data points as they move right become darker, which implies that census tracts with medium or high percentages of White individuals have more medium or high Income Per Capita.
...
The third scatter plot, which instead looked at IncomePerCap and percent White as it related to Unemployment, relayed much the same information as the plot for Service work.  However, the trend was not as robust, which corroborated the results of the correlation matrix where there was a smaller magnitude correlation between Service and IncomePerCap.
...
The fourth scatter plot looked at IncomePerCap and percent White as it related to EconomicFreedom.  There were 50 lines in this scatter plot because the data from the freedom dataset only varied at the state level.  This demonstrated how the freedom data was used to help measure IncomePerCap in the census tracts.  The lines to the right were the states with more economic freedom.  The colors  of the lines suggested that White individuals tend to live in states with higher Economic freedom and tend to have more income per capita.
...
An ANOVA test was attempted to determine if there were significant differences in income based on which ethnicity or work was the most prevalent in a census tract. The first step was identifying which ethnic variable and which work variable had the highest percentage, not necessarily the majority, in each census tract. These variables were named EthnicPlurality and WorkPlurality. It was notable that most census tracts were both plurality white and plurality professional (41484/73199).  The Levene’s test performed for both plurality variables returned p-values of 2.2E-16 (the smallest value R recognizes above zero), indicating heterogeneity of variances; in short, the ANOVA test was not performed due to this failed assumption.  However, from a more rudimentary perusal of the variables it seemed plausible that there was some relationship between the most common employment status in a census tract and the most populous ethnic group within it. 
...
BigironSphere (2018). Child Poverty in New York City: A Comprehensive Study. Retrieved March 23, 2020, from https://www.kaggle.com/bigironsphere/tutorial-maps-eda-and-models-with-nyc-census-data
...
BigironSphere (2018). Child Poverty in New York City: A Comprehensive Study
. Retrieved March 23, 2020, from https://www.kaggle.com/bigironsphere/tutorial-maps-eda-and-models-with-nyc-census-data
