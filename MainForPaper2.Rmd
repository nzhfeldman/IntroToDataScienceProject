...
hist(dat1$TotalPop, 
        col = "#a8c2fb",
        main = "Histogram of Tract Populations",
...
leveneTest(IncomePerCap ~ EthnicPlurality, data = anova_dat)
leveneTest(IncomePerCap ~ WorkPlurality, data = anova_dat)
```
...
plot(reg.summary$rsq,xlab = "# of Variables", ylab = "Rsquare", type = "l")
...
While we include all four measures of model strength above, we will focus on Cp; ... explanation of why, note that other measures lead to similar results .... Cp leads us to include all variables except for Hispanic Ethnicity, Personal Freedom, Resulatory policy, and proffessional work status.

-- From dinesh, keep? --

From the distribution graph, almost all the variables are included which was not helpful. If we were to select the best variables then it would be ‘WHITE’, 'Native', 'Asian','‘Professional’,'Office','Construction','Production',‘Unemployment’, 'Fiscal Policy'‘Personal Freedom’ and ‘service’ which is at highest R2 Value .68.For Adjusted R2, the highest adjusted R2 is obtained at .68 and this is similar to that of R2, the only difference is the variable ‘Service’ is excluded.
...
```{r run regression}
lm1 <- lm(IncomePerCap~. - Hispanic - Professional - PersonalFreedom - RegulatoryPolicy , data = dat1_varOfInterest)
summary(lm1)
```

```{r run regression}
lm2 <- lm(IncomePerCap~(. - Hispanic - Professional - PersonalFreedom - RegulatoryPolicy)^2 , data = dat1_varOfInterest)
summary(lm2)
```
