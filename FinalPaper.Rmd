...
```{r Feature Selection} 
loadPkg('leaps')
reg.best10 <- regsubsets(IncomePerCap~. , data = dat_1, nvmax = 10)  
...
```{r Regression} 
loadPkg('car')
lr <- lm(IncomePerCap  ~ . - Professional, data = dat_1) 
summary(lr)
lr
vif(lr)
coef <- coef(lr)
...
```{r} 
glr <- glm(ipc   ~ . - Professional, data = dat_1 %>% select(-TotalPop), family = "binomial") 
summary(glr)
vif(glr)
coef <- coef(glr)
```
